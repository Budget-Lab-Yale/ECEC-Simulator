...
run_scenario <- function(scenario_info, supply_params, demand_params, parent_units_list,
  ...
  if (!is.null(demand_policy$rationing)) {
    ...
    if (!is.null(scenario_info$policy_transfer) &&
        !is.na(scenario_info$policy_transfer) &&
        !scenario_info$policy_transfer %in% c('', 'baseline')) {
      stop('Transfer policies are not yet supported with quantity-limited ',
           '(rationed) demand policies: the offer-state mixture machinery does ',
           'not thread transfers through offer-conditional utilities.')
    }
