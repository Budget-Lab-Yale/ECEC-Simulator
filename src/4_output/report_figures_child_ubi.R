#------------------------------------------------------------------------------
# report_figures_child_ubi.R
#
# Adapted from report_figures.R for the child_ubi_runs scenario set:
#   baseline + Child Allowance $1K / $10K / $20K.
#
# Generates report tables and figures from child_ubi_runs output.
#
# Outputs:
...
#
# Usage:
#   Rscript src/4_output/report_figures_child_ubi.R <timestamp>
#   e.g., Rscript src/4_output/report_figures_child_ubi.R 202607061200
#------------------------------------------------------------------------------
...
ALL_SCENARIOS <- c('baseline', 'child_ubi_1k', 'child_ubi_10k', 'child_ubi_20k')
COUNTERFACTUALS <- ALL_SCENARIOS[ALL_SCENARIOS != 'baseline']

# Display labels for scenarios
SCENARIO_LABELS <- c(
  'baseline'      = 'Baseline',
  'child_ubi_1k'  = 'Child Allowance ($1K)',
  'child_ubi_10k' = 'Child Allowance ($10K)',
  'child_ubi_20k' = 'Child Allowance ($20K)'
)
...
SCENARIO_COLORS <- c(
  'Baseline'               = '#333333',
  'Child Allowance ($1K)'  = '#D4A6C8',
  'Child Allowance ($10K)' = '#B07AA1',
  'Child Allowance ($20K)' = '#7B4F73'
)
...
for (s in COUNTERFACTUALS) {
  if (grepl('^child_ubi', s)) {
    direct_by_decade[[s]] <- cost_by_decade[[s]]
    tax_by_decade[[s]]    <- rep(0, length(DECADE_NAMES))
  }
}
...
# Build data frame
GENEROSITY_POLICIES <- c('child_ubi_1k', 'child_ubi_10k', 'child_ubi_20k')
GENEROSITY_LABELS <- SCENARIO_LABELS[GENEROSITY_POLICIES]

# Flat per-child allowance amounts (2026 dollars), independent of AGI and care cost
UBI_AMOUNTS <- c(child_ubi_1k = 1000, child_ubi_10k = 10000, child_ubi_20k = 20000)

generosity_df <- bind_rows(lapply(names(UBI_AMOUNTS), function(s) {
  tibble(agi = agi_grid, subsidy = UBI_AMOUNTS[[s]], scenario = s)
})) %>%
...
fig_generosity <- ggplot(generosity_df, aes(x = agi / 1000, y = subsidy, color = scenario_label)) +
  ...
  labs(
    ...
    subtitle = 'Flat per-child allowance, no income phase-out (amount per child)',
...
cost_care_df <- bind_rows(lapply(INCOME_LEVELS, function(inc) {
  bind_rows(lapply(names(UBI_AMOUNTS), function(s) {
    tibble(care_cost = cost_grid, subsidy = UBI_AMOUNTS[[s]], scenario = s, agi = inc)
  }))
})) %>%
  mutate(
